...
@section('content')
    <div class="container" id="noteContainer">
        ...
        <div class="row">
            <div class="col-md-3">
                <ul class="list-group">
                    <a href="{{ route('notes.index') }}" class="list-group-item"><span
                                class="badge">{{ $counts['pendingCount']}}</span> Pending </a>
                    <a href="{{ route('notes.index').'?status=completed' }}" class="list-group-item"><span
                                class="badge">{{ $counts['completedCount']}}</span> Completed</a>
                    <a href="{{ route('notes.index').'?deleted=1' }}" class="list-group-item"><span
                                class="badge">{{ $counts['deletedCount']}}</span> Deleted</a>
                </ul>
            </div>
            <div class="col-md-9">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <button class="btn-block btn btn-primary" data-toggle="modal" data-target="#addNoteModal">add a
                            note
                        </button>
                        <table class="table table-striped table-hover">
                            ...
                            @foreach ($notes as $note)
                                <tr data-toggle="tooltip" title="{{$note->content}}">
                                    ...
                                    @if (!$deleted && ($status=='pending' || !$status))
                                        <td>
                                            <button class="btn btn-sm" v-on:click="completeNote('{{$note->id}}')"><span
                                                        class="glyphicon glyphicon-ok"></span></button>
                                            <button class="btn btn-sm" v-on:click="deleteNote('{{$note->id}}')"><span
                                                        class="glyphicon glyphicon-trash"></span></button>
                                            {{-- <button class="btn btn-sm" v-on:click="editNote('{{$note->id}}')"><span class="glyphicon glyphicon-edit"></span></button> --}}
                                        </td>
                                    @elseif ($deleted)
                                        <td>
                                            <button class="btn btn-sm" v-on:click="deleteNoteForce('{{$note->id}}')">
                    ...
                    <div class="modal-body">
                        <form role="form">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" v-model="classification"
                                       placeholder="Enter classification">
                            </div>
                            <div class="form-group">
                                <label for="content"><span class="glyphicon glyphicon-book"></span> Content</label>
                                <textarea class="form-control" v-model="content" rows="10"
